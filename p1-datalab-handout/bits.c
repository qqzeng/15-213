 ...
 */
int isTmax(int x) {
  // return !(~((x+1) + x) + !(x+1)); // the second part is used to differ from -1.
  return !(((x+1) + x) + 1 + !(x+1));
}
/* 
...
int allOddBits(int x) {
  int allOdd = (0xAA << 0x08 ) | 0xAA; 
  allOdd = allOdd << 0x10 | allOdd;
...
int isAsciiDigit(int x) {
  int sign = 1 << 31;
  int right = ~(sign | 0x39); // if x + right overflow(<0), then x > 0x39
  int left = ~0x30; // if x + left does not overflow, then x < 0x30
  int ofRight = sign & (right + x) >> 31;
  int ofLeft = sign & (left + x + 1) >> 31;
  return !(ofRight | ofLeft);
}
/* 
...
int conditional(int x, int y, int z) {
  x = ~(!!x) + 1; // if x != 0, then x is all 1, otherwise, all 0
  return (x & y) | (~x & z);
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  // int signSame = (!(signx^signy)) & ((~y + x) >> 31);
  int signSame = (!(signx^signy)) & !!((~y + x)& (1 << 31));
...
int howManyBits(int x) {
  // reference from: https://blog.csdn.net/blackturtle/article/details/70177180
  // positive numbers equal to themselves
  // and convert negative numbers x to |x|-1, the bits of number |x|-1 needed equals to the original negative one.
  int positive = x ^ ( x >> 31);
